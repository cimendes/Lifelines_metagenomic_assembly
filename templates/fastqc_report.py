...
def check_summary_health(summary_file, **kwargs):
    ...
    fail_if_fail = kwargs.get("fail_sensitive", [
        "Per base sequence quality",
        "Overrepresented sequences",
        "Sequence Length Distribution",
        "Per sequence GC content"
    ...
    warning_if_warning = kwargs.get("warning_fail_sensitive", [
        "Per base sequence quality",
        "Overrepresented sequences",

    ])

    warning_if_fail = kwargs.get("warning_must_pass", [
    ...
    for cat, test in summary_info.items():
        ...
        # WARNINGS
        # Check for fail sensitive
        if cat in warning_if_warning and test != "PASS": #warning?
